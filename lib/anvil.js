...
function loader (mcData, Item) {
  function combine (itemOne, itemTwo, creative, renamedName) {
    ...
    try {
      if (!combinePossible(itemOne, itemTwo) && itemTwo !== null) return { xpCost: 0, invalid: true } // throws if not possible
      let cost = getBaseCost(itemOne) + getBaseCost(itemTwo)// anvilUsesData[getAnvilUses(itemOne, itemTwo)]
      if (rename) {
        const renameCost = getRenameCost(itemOne)
        if (renameCost === -1) return { xpCost: 0, invalid: true }
        cost += renameCost
      }
      ...
      if (itemTwo && (itemTwo.name === itemOne.name || itemTwo.name === 'enchanted_book')) {
        ...
        if (enchantCost === 0 && !rename && itemOne.metadata === 0) return { xpCost: 0, invalid: true } // no change
        cost += enchantCost
      }
      let finalItem = null // defaults to null for if first item is air, TODO: just be the original item with new name
      if (itemOne) {
        finalItem = new Item(itemOne.type, itemOne.count, itemOne.metadata - data.fixedDurability, itemOne.nbt)
        finalItem.setEnchants(data.finalEnchs, getAnvilUses(itemOne, itemTwo))
      }
  ...
  function getAnvilUses (itemOne, itemTwo) {
    const baseOne = getBaseCost(itemOne)
    const baseTwo = getBaseCost(itemTwo)
    const baseUsed = baseOne !== baseTwo ? Math.max(baseOne, baseTwo) : baseOne
    return anvilUsesData.indexOf(baseUsed) + 1
  }
    ...
    for (const ench of itemTwoEnch) {
      ...
      if (enchOnItemOne === undefined) { // first item doesn't have this ench
        ...
        if (itemOneLevel === itemTwoLevel) { // add check for max level > combined level
          ...
          finalEnchs.push({ name: ench.name, lvl: finalLevel })
        // return
