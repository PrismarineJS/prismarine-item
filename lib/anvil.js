 ...
 */
  ...
  function repairCost (itemOne, itemTwo) {
    ...
    if (itemTwo.name === itemOne.name) {
      fixedDurability = (0.12 * maxDurability) + itemTwo.metadata
      xpLevelCost = 2
      usedMats = 1
  ...
  function combinePossible (itemOne, itemTwo) {
    if (Object.keys(itemTwo).length === 0) return // rename
    if (itemOne.name === 'enchanted_book' && itemTwo.name !== 'enchanted_book') throw new Error('Can only combine book with book')
    else if (itemOne.name === 'enchanted_book') return // book isn't fixable
    const fixMaterials = fixData.find(entry => entry[0].includes(itemOne.name))[1].concat(['enchanted_book', itemOne.name]) // each entry in fixdata is [fixablethings, fixablewith]
