...
function loader (mcData, Item) {
  function combine (itemOne, itemTwo, creative, renamedName) {
    ...
    if (!combinePossible(itemOne, itemTwo) && itemTwo !== null) return { xpCost: 0, invalid: true }
    ...
    if (itemOne) {
      ...
      const anvilUses = getAnvilUses(itemOne, itemTwo) + 1
      finalItem.setEnchants(data.finalEnchs, anvilUses)
    }
  ...
  function getAnvilUses (itemOne, itemTwo) {
    ...
    return Item.toAnvilUses(Math.max(baseOne, baseTwo))
  }
