...
function loader (mcData, Item) {
  function combine (itemOne, itemTwo, creative, rename) {
    const data = {}
    try {
      if (!combinePossible(itemOne, itemTwo)) return { xpCost: 0, invalid: true } // throws if not possible
      ...
      const finalItem = new Item(itemOne.id, 1, itemOne.metadata + data.fixedDurability, null)
      finalItem.setEnchants(data.finalEnchs, anvilUses.indexOf(baseCost(itemOne)))
      return { xpCost: cost, item: finalItem }
    } catch (err) {
