...
function loader (mcData, Item) {
  function combine (itemOne, itemTwo, creative, renamedName) {
    ...
    if (cost > 39 && onlyRename) cost = 39 // TODO: add tests for this
    ...
    if (itemOne) {
      ...
      /* if (!(rename && cost === 1)) */ finalItem.setRepairCost(repairCost) // Renaming does not increase the prior work penalty.
    }
 ...
 */
  function combineEnchants (itemOne, itemTwo, creative) {
    ...
    if (finalEnchs === null && itemTwoEnch === null) return { xpLevelCost: 0, finalEnchs: [] }
    let xpLevelCost = 0
    for (const ench of itemTwoEnch) {
      ...
      if (enchOnItemOne === undefined) { // first item doesn't have this ench
        ...
        if (exclude.some(({ name }) => finalEnchs.find(x => x.name === name))) { // has an excluded enchant
          xpLevelCost++
        } else {
