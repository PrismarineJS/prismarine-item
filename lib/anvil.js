...
function loader (mcData, Item) {
  function combine (itemOne, itemTwo, creative, renamedName) {
    ...
    try {
      ...
      if (itemOne) {
        finalItem = new Item(itemOne.type, itemOne.count, itemOne.metadata + data.fixedDurability, null) // replace null with itemOne.nbt
        finalItem.setEnchants(data.finalEnchs, anvilUses.indexOf(baseCost(itemOne)))
      }
