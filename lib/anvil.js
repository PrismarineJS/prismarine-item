...
function loader (mcData, Item) {
  function combine (itemOne, itemTwo, creative, renamedName) {
    ...
    if (itemOne.getDurabilityUsed() !== 0) {
      onlyRename = false
    ...
    cost += (getBaseCost(itemOne) + getBaseCost(itemTwo))
    ...
    if (itemOne) {
      ...
      finalItem.setDurabilityUsed(itemOne.getDurabilityUsed() - data.fixedDurability)
    }
 ...
 */
  function combineEnchants (itemOne, itemTwo, creative) {
    ...
    const finalEnchs = itemOne.getEnchants()
    ...
    for (const ench of itemTwoEnch) {
      ...
      if (!(itemOne.name === 'enchanted_book' && rightIsBook) && !categoryItems.includes(itemOne.name) && !creative) continue
  ...
  function getRepairCost (itemOne, itemTwo) {
    ...
    const durabilityLost = itemOne.getDurabilityUsed()
    const [, fixMaterials] = fixData.find(fixEntry => fixEntry[0].includes(itemOne.name))
    ...
    if (itemTwo.name === itemOne.name) {
      ...
      results = {
        fixedDurability: itemOne.metadata < possibleFixedDura ? itemOne.getDurabilityUsed() : possibleFixedDura,
        xpLevelCost: 2,
        usedMats: 1
      }
      ...
      if (itemTwo.count > matsToFullyRepair) { // takeall of itemTwo
        results = {
          fixedDurability: maxItemDura[itemOne.name] - itemOne.getDurabilityUsed(),
          xpLevelCost: matsToFullyRepair, // 1 exp lvl per mat used
          usedMats: matsToFullyRepair
        }
      } else if (itemOne && itemTwo) {
        results = {
          fixedDurability: Math.min(itemOne.getDurabilityUsed(), durabilityFixedPerMat * itemTwo.count),
