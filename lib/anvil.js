function loader (registry, Item) {
  ...
  function combineEnchants (itemOne, itemTwo, creative) {
    ...
    for (const ench of itemTwoEnch) {
      ...
      let { exclude, maxLevel, category, weight, rarity, supportedItems } = registry.enchantmentsByName[ench.name]
      const multiplier = getMultipliers(weight, rightIsBook, rarity)
      const acceptsItem = supportedItems !== undefined
        ? supportedItems.includes(itemOne.name)
        : (registry.itemsByName[itemOne.name].enchantCategories ?? []).includes(category)
      if (!(itemOne.name === 'enchanted_book' && rightIsBook) && !acceptsItem && !creative) continue
  ...
  // Rarity determines anvil cost; older data only exposes its usual weight.
  function getMultipliers (weight, isBook, rarity) {
